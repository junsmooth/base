...
public class DataUtils {
	...
	public final static <T> List<T> packingData(List<String> filedNameList,
			List<T> dataList) {
		...
		for (Object obj : dataList) {
			...
			// DataUtils.class.getGenericSuperclass();
			...
			try {
				...
				if ("".equals(newDate)) {
					...
					resultList.add(new BigDecimal(dayValue).setScale(3,
	...
	public static List objectList2JSonList(List li, Object... object) {
		...
		try {
			...
			for (Object o : li) {
				...
				Field datetime = o.getClass().getDeclaredField(
						object[0].toString());
				Field value = o.getClass().getDeclaredField(
						object[1].toString());
		...
		try {
			for (Object obj : dataList) {
				if (type != null) {
					...
					Method deM = obj.getClass()
							.getMethod("setdE", de.getType());
					Method dhM = obj.getClass()
							.getMethod("setdH", dh.getType());
					Method dnM = obj.getClass()
							.getMethod("setdN", dn.getType());
					double deValue = new BigDecimal((Double) de.get(obj) * 1000)
							.setScale(2, BigDecimal.ROUND_HALF_UP)
							.doubleValue();
					double dhValue = new BigDecimal((Double) dh.get(obj) * 1000)
							.setScale(2, BigDecimal.ROUND_HALF_UP)
							.doubleValue();
					double dnValue = new BigDecimal((Double) dn.get(obj) * 1000)
							.setScale(2, BigDecimal.ROUND_HALF_UP)
							.doubleValue();
					...
					value.setAccessible(true);
					Method method = obj.getClass().getMethod("setValue",
							value.getType());
