...
public class JRXService {
	...
	public PageList getJRXMonitorPosition(TableParam param) {
		...
		MonitoringType t = commonDao.findUniqueBy(MonitoringType.class, "code",
				Constants.JCD_JRX);
		List<MonitoringPoint> jrxPointList = commonDao.findByCriterions(
				MonitoringPoint.class, Restrictions.eq("type.id", t.getId()));
		PagedQuery pq = new PagedQuery(JRX.class, param.getPage(),
				param.getRows());
		...
		if (StringUtils.isNotEmpty(param.getStr())) {
			...
			criteria.add(Restrictions.in("monitoringPosition",
					positions.toArray()));
		}
...
//	public  static void setDecimalDigits(List<Object> rows) {
//		for(Object obj:rows){
